...
for i in $(seq 1 $((line_count / 11))); do
    ...
    NAME="${my_array[$((8 + next_vehicle))]}" \
    ENDURANCE="${my_array[$((9 + next_vehicle))]}" \
    SENSOR_RANGE="${my_array[$((10 + next_vehicle))]}" \


  # Increment next_vehicle by 4 to move to the next set of variables
  ((next_vehicle += 11)) #we divide by 8 because of the number of
